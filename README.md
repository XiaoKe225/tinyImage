...
**纯本地桌面图片压缩工具**（Windows · Tauri 2 + Rust）· 当前版本 **2.1.3**。
...
- **图钉置顶**：窗口右上角图钉图标，一键将窗口固定在最前，方便拖图时始终可见（状态会记住）
...
2. 下载 **`TinyImage_2.1.3_x64-setup.exe`**（或同版本 `*-protected.zip`）
3. 安装并启动 **TinyImage**
...
**图钉置顶**：点窗口右上角 **图钉图标** 可将窗口固定在最前；再次点击取消。
...
```bash
npm run test:rust
npm run verify:pin-local    # 图钉置顶 · 本机五连验
npm run verify:speed-local  # 压缩加速 · 体积指纹五连验
```
...
| `scripts/` | Pack、混淆、本机验收（`build.ps1`、`verify-*-local.ps1`、`cdp-eval.mjs`） |
| `docs/` | 打赏图、维护说明 |
| `legacy-electron/` | 旧版对照（不演进） |
| `release/` | 本地 Pack 产物（gitignore） |
